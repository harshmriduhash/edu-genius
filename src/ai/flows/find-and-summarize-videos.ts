...
export type FindAndSummarizeVideosInput = z.infer<
  typeof FindAndSummarizeVideosInputSchema
>;
...
const VideoResultSchema = z.object({
  ...
  summary: z
    .string()
    .describe("An AI-generated summary of the video description."),
});

const FindAndSummarizeVideosOutputSchema = z.object({
  videos: z
    .array(VideoResultSchema)
    .describe("A list of found videos with their summaries."),
});
export type FindAndSummarizeVideosOutput = z.infer<
  typeof FindAndSummarizeVideosOutputSchema
>;
...
const findAndSummarizeVideosFlow = ai.defineFlow(
  ...
  async ({ topic, apiKey }) => {
    ...
    const youtubeResponse = await fetch(
      `${YOUTUBE_API_URL}?${searchParams.toString()}`
    );
    ...
    const summarizedVideos = await Promise.all(
      youtubeData.items.map(async (item: any) => {
        ...
        if (description && description.trim().length > 50) {
          // Only summarize if description is meaningful
          try {
            ...
            console.warn(
              `Could not summarize description for video ${item.id.videoId}`,
              e
            );
...
export async function findAndSummarizeVideos(
  input: FindAndSummarizeVideosInput
): Promise<FindAndSummarizeVideosOutput> {
